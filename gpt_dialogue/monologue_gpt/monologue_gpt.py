...
class MonologueGPT(LanguageModel):
    ...
    def load(
        ...
        if not os.path.exists(model_checkpoint) and \
                not model_checkpoint in self._SUPPORTED_MODELS:
            logger.error(
        ...
        tokenizer_args = {
            k:v for k,v in tokenizer_args.items() if v != None
        }
        # Load tokenizer
        ...
        # Remove all the None args
        model_args = {
            k:v  for k,v in model_args.items() if v != None
        }
        # Load the model
        ...
        if len(model_args) == 1 and len(tokenizer_args) == 1 and \
                model_args["pretrained_model_name_or_path"] in self._SUPPORTED_MODELS and \
                tokenizer_args["pretrained_model_name_or_path"] in self._SUPPORTED_MODELS:
            logger.warning(
