...
# @Last Modified time: 2022-08-01 16:41:47
...
class TurnGPT(pl.LightningModule):
    ...
    def _initialize_tokenizer(self):
        ...
        self.tokenizer = SpokenDialogueTokenizer(self.pretrained_model_name_or_path)
    ...
    def _initialize_special_embeddings(self, tokens=["!", "?", "."]):
        ...
        with torch.no_grad():
            ...
            avg_embedding = self.transformer.transformer.wte(ids).mean(0)
            # Assign the average as the embedding for the eos special token.
            self.transformer.transformer.wte.weight.data[
                self.tokenizer.eos_token_id] = avg_embedding
        print(f"Initialized special wte for {self.tokenizer.eos_token} to average of {tokens}")
        print(f"This will make the {self.tokenizer.eos_token} semantically closer to {tokens}")
