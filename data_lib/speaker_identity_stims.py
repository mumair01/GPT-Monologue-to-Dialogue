...
class SpeakerIdentityStimuliDataset:
    ...
    def __call__(self, variant: str, save_dir: str):
        ...
        partial_save_path = os.path.join(
            save_dir, f"speaker_identity_stimuli_{variant}"
        )
    ...
    def _process_special_labels_variant(self, cha_path):
        ...
        # Create and apply normalizer sequence.
        normalizer_seq = create_normalizer_sequence(custom_regex="(\*)")
        conv = [
            normalizer_seq(toks)
            for line in conv
            for toks in re.split(r"\. |\?|\t+|:", line)
        ...
        for sp, line in zip(speakers, lines):
            sp = self.special_labels_variant_labels["speaker_base"].format(
                sp[-1]
            )
    ...
    def _save_dataset_as_csv(self, csv_path, dataset):
        ...
        dataset_df = pd.DataFrame(
            dataset, columns=["convID", "convName", "turnNo", "Utterance"]
        )
...
if __name__ == "__main__":
    ...
    dataset(variant=args.variant, save_dir=args.outdir)
