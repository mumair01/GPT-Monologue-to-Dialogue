...
@pytest.mark.parametrize(
    "s, remove_words, expected",
    [
        (
            "<SP1> needs to be replaced!!!",
            ["<SP1>", "needs"],
            "to be replaced!!!",
        )
    ],
...
def test_remove_words_from_string(s, remove_words, expected):
    s2 = remove_words_from_string(s=s, remove_words=remove_words)
    assert s2 == expected


@pytest.mark.parametrize(
    "s, word, replacement, expected",
    [
...
def test_replace_word_from_string(s, word, replacement, expected):
    s2 = replace_word_from_string(s=s, word=word, replacement=replacement)
    assert s2 == expected
